...
#include "imgproc/fast_atan2.h"
#include "types.h"

Profiler* lsd_profiler;
...
/** Set which atan2 function to use */
#define CUR_ATAN2 fast_atan2 // fast_atan2 / table_based_atan2 // atan2
/*----------------------------------------------------------------------------*/
...
static void error(std::string msg) {
  std::cout << "LSD Error: " << msg << std::endl;
  exit(EXIT_FAILURE);
}

/*----------------------------------------------------------------------------*/
/** Table-based atan2 function.
  */
static float* atan2_table = NULL;
static const int numberOfAngles = 45;  /// In only the first 45 degree of one quadrant.
 ...
 */
inline static int double_equal(double a, double b) {
  #ifdef TIMING_RANDOMS
    ++random_iter[0];
    struct timespec start_inner, end_inner;
    clock_gettime(GET_CLOCK_METHOD, &start_inner);
  #endif
  ...
  if (abs_max < DBL_MIN)
    abs_max = DBL_MIN;
  #ifdef TIMING_RANDOMS
    clock_gettime(GET_CLOCK_METHOD, &end_inner);
    random_time[0] += difference(start_inner, end_inner).tv_nsec / 1000000.0;
  #endif
...
inline static double dist(double x1, double y1, double x2, double y2) {
    ...
    ++dist_iter;
    clock_gettime(GET_CLOCK_METHOD, &start_inner);
    double final = sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
    clock_gettime(GET_CLOCK_METHOD, &end_inner);
    dist_time += difference(start_inner, end_inner).tv_nsec / 1000000.0;
    return final;
  #else
...
static void enlarge_ntuple_list(ntuple_list n_tuple) {
  /* check parameters */
  assert(n_tuple == NULL || n_tuple->values == NULL || n_tuple->max_size == 0);

  /* duplicate number of tuples */
  n_tuple->max_size *= 2;

  /* realloc memory */
  n_tuple->values = (double *) realloc(
      (void *) n_tuple->values,
 ...
 */
static image_float new_image_float_ptr(unsigned int xsize, unsigned int ysize,
                                         float * data) {
  image_float image;
...
/** float image data type
 ...
 with x and y integer.
 */
typedef struct image_float_plus_s {
  ImageDataPrecision * data;
  ImageDataPrecision * cos_data;
  ImageDataPrecision * sin_data;
  unsigned int xsize, ysize;
}* image_float_plus;
...
/** Free memory used in image_float 'i'.
 */
static void free_image_float_plus(image_float_plus i) {
  assert(i == NULL || i->data == NULL);
  free((void *) i->data);
  free((void *) i->cos_data);
  free((void *) i->sin_data);
  free((void *) i);
}
...
/** Create a new image_float of size 'xsize' times 'ysize'.
 */
static image_float_plus new_image_float_plus(unsigned int xsize, unsigned int ysize) {
  image_float_plus image;

  /* check parameters */
  assert(xsize == 0 || ysize == 0);

  /* get memory */
  image = (image_float_plus) malloc(sizeof(struct image_float_plus_s));
  if (image == NULL)
    error("not enough memory.");
  image->data = (float *) calloc((size_t) (xsize * ysize), sizeof(float));
  image->cos_data = (float *) calloc((size_t) (xsize * ysize), sizeof(float));
  image->sin_data = (float *) calloc((size_t) (xsize * ysize), sizeof(float));
  if (image->data == NULL)
    error("not enough memory.");
...
static void gaussian_kernel(ntuple_list kernel, double sigma, double mean) {
  ...
  /* check parameters */
  assert(kernel == NULL || kernel->values == NULL);
  assert(sigma <= 0.0);
...
/** Scale the input image 'in' by a factor 'scale' by Gaussian sub-sampling.
 ...
 in the y axis.
 */
static image_float gaussian_sampler(image_float in, double scale,
                                     double sigma_scale) {
  image_float aux, out;
  ntuple_list kernel;
  ...
  /* check parameters */
  assert(in == NULL || in->data == NULL || in->xsize == 0 || in->ysize == 0);
  assert(scale <= 0.0);
  assert(sigma_scale <= 0.0);

  /* compute new image size and get memory for images */
  assert(in->xsize * scale > (double) UINT_MAX
  ...
  for (x = 0; x < aux->xsize; ++x) {
    ...
    xc = (int) (xx + 0.5);  // Instead of floor
  ...
  for (y = 0; y < out->ysize; ++y) {
    ...
    yc = (int) (yy + 0.5);  //Instead of floor
...
/*|--****************************** Gradient ******************************--|*/
/*|--**********************************************************************--|*/
static image_float_plus ll_angle_grad(image_float in, EyeMARS::Matrix2d<short> &sobel,
                                      double threshold, struct coorlist **list_p,
                                      void **mem_p,
                                      image_float * modgrad, unsigned int n_bins) {

  #ifdef TIMING_ll_angle_grad
    if(lsd_profiler){
      lsd_profiler->Start("ll_angle_grad");
    }
  #endif

  image_float_plus g;
  unsigned int n, p, x, y, adr, i;
  double com1, com2, gx, gy, norm, norm2, ang;
  ...
  g = new_image_float_plus(in->xsize, in->ysize);

  // Get pointer to sobel data
  short* sd = sobel.data();
  ...
  *mem_p = (void *) list;
  ...
  for (x = 0; x < p - 1; ++x) {
    for (y = 0; y < n - 1; ++y) {
      ...
      gx = sd[y*2*p+2*x];    // copy x component
      gy = sd[y*2*p+2*x+1];  // copy y component

      norm2 = gx * gx + gy * gy;
      norm = sqrt(norm2*0.25); /* gradient norm */
      ...
      if (norm <= threshold) /* norm too small, gradient no defined */
        ...
        ang = CUR_ATAN2(gx, -gy);
        g->data[adr] = ang;
        g->cos_data[adr] = -gy/norm2; // cos(ang);
        g->sin_data[adr] = gx/norm2; // sin(ang);
  ...
  *list_p = start;
  ...
  #ifdef TIMING_ll_angle_grad
    if(lsd_profiler){
      lsd_profiler->Stop("ll_angle_grad");
    }
  #endif
 ...
 */
static image_float_plus ll_angle(image_float in, double threshold,
                             struct coorlist **list_p, void **mem_p,
                             image_float * modgrad, unsigned int n_bins) {
  #ifdef TIMING_ll_angle
    if(lsd_profiler){
      lsd_profiler->Start("ll_angle_grad");
    }
  #endif
  ...
  for (x = 0; x < p - 1; ++x) {
    for (y = 0; y < n - 1; ++y) {
      ...
      if (norm <= threshold) /* norm too small, gradient no defined */
        ...
        ang = CUR_ATAN2(gx, -gy);
        g->data[adr] = ang;
        g->cos_data[adr] = cos(ang);
        g->sin_data[adr] = sin(ang);
 ...
 */
inline static int isaligned(int x, int y, image_float_plus angles, double theta,
                     double prec) {
  #ifdef TIMING_RANDOMS
    ++random_iter[1];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
  #endif
  double a;
  ...
  #ifdef TIMING_RANDOMS
    clock_gettime(CLOCK_REALTIME_COARSE, &end_inner);
    random_time[1] += difference(start_inner, end_inner).tv_nsec / 1000000.0;
  #endif
  return theta <= prec;
}
 ...
 */
inline static double angle_diff(double a, double b) {
  #ifdef TIMING_RANDOMS
    ++random_iter[2];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
 ...
 */
inline static double angle_diff_signed(double a, double b) {
  #ifdef TIMING_RANDOMS
    ++random_iter[3];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
...
/** Computes the natural logarithm of the absolute value of
 ...
 q6 = 2.50662827511.
 */
inline static double log_gamma_lanczos(double x) {
  #ifdef TIMING_RANDOMS
    ++random_iter[4];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
 ...
 */
inline static double log_gamma_windschitl(double x) {
  #ifdef TIMING_RANDOMS
    ++random_iter[5];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
...
static double nfa(int n, int k, double p, double logNT) {
  ...
  #ifdef TIMING_OLD
    clock_gettime(CLOCK_REALTIME_COARSE, &end_inner);
    nfa_time += difference(start_inner, end_inner).tv_nsec / 1000000.0;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
  #endif
  /* trivial cases */
  if (n == 0 || k == 0)
  ...
  for (i = k + 1; i <= n; ++i) {
    ...
    if (bin_term < 1.0) {
      ...
      err = term
          * ((1.0 - pow(mult_term, (n - i + 1))) / (1.0 - mult_term)
              - 1.0);
 ...
 */
inline static void rect_copy(struct rect * in, struct rect * out) {
  #ifdef TIMING_RANDOMS
    ++random_iter[6];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
...
/** Interpolate y value corresponding to 'x' value given, in
 ...
 - x  <= x2
 */
inline static double inter_low(double x, double x1, double y1, double x2, double y2) {
  #ifdef TIMING_RANDOMS
    ++random_iter[7];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
...
/** Interpolate y value corresponding to 'x' value given, in
 ...
 - x  <= x2
 */
inline static double inter_hi(double x, double x1, double y1, double x2, double y2) {
  #ifdef TIMING_RANDOMS
    ++random_iter[8];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
 ...
 */
inline static void ri_del(rect_iter * iter) {
  #ifdef TIMING_RANDOMS
    ++random_iter[9];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
 ...
 */
inline static int ri_end(rect_iter * i) {
  #ifdef TIMING_RANDOMS
    ++random_iter[10];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
  ...
  #ifdef TIMING_RANDOMS
    clock_gettime(CLOCK_REALTIME_COARSE, &end_inner);
    random_time[10] += difference(start_inner, end_inner).tv_nsec / 1000000.0;
  #endif
  return (int) (i->x) > i->vx[2];
}
...
static void ri_inc(rect_iter * i) {
  ...
  #ifdef TIMING_RANDOMS
    clock_gettime(CLOCK_REALTIME_COARSE, &end_inner);
    random_time[11] += difference(start_inner, end_inner).tv_nsec / 1000000.0;
  #endif
    /* new y */
    i->y = (int) ceil(i->ys);
  }
...
static rect_iter * ri_ini(struct rect * r) {
  ...
  #ifdef TIMING_RANDOMS
    clock_gettime(CLOCK_REALTIME_COARSE, &end_inner);
    random_time[12] += difference(start_inner, end_inner).tv_nsec / 1000000.0;
  #endif
  return i;
}
 ...
 */
static double rect_nfa(struct rect * rec, image_float_plus angles, double logNT) {
  #ifdef TIMING_RANDOMS
    ++random_iter[13];
    struct timespec start_inner, end_inner;
    clock_gettime(CLOCK_REALTIME_COARSE, &start_inner);
  ...
  ri_del(i); /* delete iterator */
  #ifdef TIMING_RANDOMS
  double finald = nfa(pts, alg, rec->p, logNT);
    clock_gettime(CLOCK_REALTIME_COARSE, &end_inner);
    random_time[13] += difference(start_inner, end_inner).tv_nsec / 1000000.0;
    return finald;
  #else
...
static double get_theta(struct point * reg, int reg_size, double x, double y,
                        image_float modgrad, double reg_angle, double prec) {
  #ifdef TIMING_get_theta
    if(lsd_profiler){
      lsd_profiler->Start("get_theta");
    }
    // SimpleTimer timer;
    // function_timers.insert(std::pair<std::string, SimpleTimer>("get_theta", timer));
    // function_timers.find("get_theta")->second.Start();
  #endif
  ...
  assert (reg == NULL);
  assert (reg_size <= 1);
  assert (modgrad == NULL || modgrad->data == NULL);
  assert (prec < 0.0);
  ...
  lambda = 0.5
      * (Ixx + Iyy - sqrt((Ixx - Iyy) * (Ixx - Iyy) + 4.0 * Ixy * Ixy));

  /* compute angle */
  theta =
      fabs(Ixx) > fabs(Iyy) ?
          CUR_ATAN2(lambda - Ixx, Ixy) : CUR_ATAN2(Ixy, lambda - Iyy);
  ...
  #ifdef TIMING_get_theta
    if (lsd_profiler) {
      lsd_profiler->Stop("get_theta");
    }
    // function_timers.find("get_theta")->second.Stop();
  #endif
 ...
 */
static void region2rect(struct point * reg, int reg_size, image_float modgrad,
                        image_float_plus angles,
  ...
  double x, y, dx, dy, l, w, theta, weight, sum, l_min, l_max, w_min, w_max, sumdx, sumdy;
  int i;
   ...
   */
#ifndef USE_OUR_ANGLE
  sumdx = sumdy = x = y = sum = 0.0;
  for (i = 0; i < reg_size; ++i) {
    weight = modgrad->data[reg[i].x + reg[i].y * modgrad->xsize];
...
#else
  // std::cout << "Using our method" << std::endl;
   x = y = sum = 0.0;
  for (i = 0; i < reg_size; ++i) {
    weight = modgrad->data[reg[i].x + reg[i].y * modgrad->xsize];
    x += (double) reg[i].x;
  ...
  #ifdef TIMING_OLD
    clock_gettime(GET_CLOCK_METHOD, &end_inner);
    region2rect_time += difference(start_inner, end_inner).tv_nsec / 1000000.0;
  #endif
}
 ...
 */
static void region_grow(int x, int y, image_float_plus angles, struct point * reg,
  ...
  *reg_angle = angles->data[x + y * angles->xsize]; /* region's angle */
  sumdx = angles->cos_data[x + y * angles->xsize];
  sumdy = angles->sin_data[x + y * angles->xsize];
  used->data[x + y * used->xsize] = USED;

  // only update reg_angle after x have been added
  int window =16;
  int count = 0;

  /* try neighbors as new region points */
  // float test_angles[16];
  // int aligned_angles[16];
  // int test_x[16];
  // int test_y[16];
  // int ac = 0;
  // int j = 0;
      ...
      for (yy = reg[i].y - 1; yy <= reg[i].y + 1; ++yy) {
        if (xx >= 0 && yy >= 0 && xx < (int) used->xsize
            ...
            && isaligned(xx, yy, angles, *reg_angle, prec)) {
                ...
                sumdx += angles->cos_data[xx + yy * angles->xsize];
                sumdy += angles->sin_data[xx + yy * angles->xsize];

                if (++count >= window) {
                  *reg_angle = CUR_ATAN2(sumdy, sumdx);
                  count = 0;
                }
        }
      }
  ...
  #ifdef TIMING_OLD
    clock_gettime(GET_CLOCK_METHOD, &end_inner);
    region_grow_time += difference(start_inner, end_inner).tv_nsec / 1000000.0;
  #endif
}
 ...
 */
static double rect_improve(struct rect * rec, image_float_plus angles, double logNT,
                           double log_eps) {
  #ifdef TIMING_OLD
    struct timespec start_inner, end_inner;
    ++rect_improve_iter;
  ...
  #ifdef TIMING_OLD
    clock_gettime(GET_CLOCK_METHOD, &end_inner);
    rect_improve_time += difference(start_inner, end_inner).tv_nsec / 1000000.0;
    clock_gettime(GET_CLOCK_METHOD, &start_inner);
  #endif
  if (log_nfa > log_eps)
    return log_nfa;
...
static int reduce_region_radius(struct point * reg, int * reg_size,
                                image_float modgrad, double reg_angle,
                                double prec, double p, struct rect * rec,
                                image_char used, image_float_plus angles,
                                double density_th) {
  #ifdef TIMING_OLD
    struct timespec start_inner, end_inner;
    ++reduce_region_radius_iter;
    clock_gettime(GET_CLOCK_METHOD, &start_inner);
  #endif
  ...
  #ifdef TIMING_OLD
    clock_gettime(GET_CLOCK_METHOD, &end_inner);
    reduce_region_radius_time += difference(start_inner, end_inner).tv_nsec / 1000000.0;
    clock_gettime(GET_CLOCK_METHOD, &start_inner);
  #endif
...
static int refine(struct point * reg, int * reg_size, image_float modgrad,
                  ...
                  image_char used, image_float_plus angles, double density_th) {
  #ifdef TIMING_OLD
    struct timespec start_inner, end_inner;
    ++refine_iter;
    clock_gettime(GET_CLOCK_METHOD, &start_inner);
  ...
  #ifdef TIMING_OLD
    clock_gettime(GET_CLOCK_METHOD, &end_inner);
    refine_time += difference(start_inner, end_inner).tv_nsec / 1000000.0;
    clock_gettime(GET_CLOCK_METHOD, &start_inner);
  #endif
  /* if the density criterion is satisfied there is nothing to do */
  if (density >= density_th)
  ...
  tau = 2.0
      * sqrt(
          (s_sum - 2.0 * mean_angle * sum) / (double) n
  ...
  if (density < density_th)
    return reduce_region_radius(reg, reg_size, modgrad, reg_angle, prec, p, rec,
...
/*----------------------------------------------------------------------------*/
/** LSD full interface. */
double* LineSegmentDetection_gradient(int *n_out, float *img, EyeMARS::Matrix2d<short> &sobel, int X, int Y,
                              double scale, double sigma_scale, double quant,
                              double ang_th, double log_eps, double density_th,
                              int n_bins, int **reg_img, int *reg_x,
                              int *reg_y, Profiler* prof) {

  lsd_profiler = prof;

  #ifdef TIMING_LineSegmentDetection_gradient
    if (prof) {
      prof->Start("LineSegmentDetection_gradient");
    }
  #endif
  image_float modgrad;// = // new_image_float(X, Y);
  struct timespec start_t, end_t, start_ll_angle, end_ll_angle;
  image_float image;
  ntuple_list out = new_ntuple_list(7);
  double *return_value;
  image_float scaled_image;
  image_float_plus angles;
  image_char used;
  ...
  if (sobel.data() != NULL && USE_OUR_SOBEL) {
    ...
    if (scale != 1.0) {
      ...
      angles = ll_angle(scaled_image, rho, &list_p, &mem_p, &modgrad,
                        (unsigned int) n_bins);
  ...
  for (; list_p != NULL; list_p = list_p->next) {
     ...
     because we are only interested in the exact NOTDEF value */
    {
       ...
       */
      if (!refine(reg, &reg_size, modgrad, reg_angle, prec, p, &rec, used,
                  angles, density_th))
        continue;
      ...
      add_7tuple(out, rec.x1, rec.y1, rec.x2, rec.y2, rec.width, rec.p,
                 log_nfa);
  ...
  *n_out = (int) (out->size);
  ...
  #ifdef TIMING_LineSegmentDetection_gradient
    if (prof) {
      prof->Stop("LineSegmentDetection_gradient");
    }
  #endif

  return return_value;
}
/** LSD interface: dont take in precomputed sobel */
double* LineSegmentDetection(int *n_out, float *img, int X, int Y,
                              double scale, double sigma_scale, double quant,
                              double ang_th, double log_eps, double density_th,
                              int n_bins, int **reg_img, int *reg_x,
                              int *reg_y,  Profiler* prof) {
  EyeMARS::Matrix2d<short> sobel(0, 0, NULL);
  return LineSegmentDetection_gradient( n_out,  img, sobel, X,  Y,
                               scale,  sigma_scale,  quant,
                               ang_th,  log_eps,  density_th,
                               n_bins,  reg_img,  reg_x,
                               reg_y, prof);
}
/*----------------------------------------------------------------------------*/
/** LSD Simple Interface with Scale and Region output. */
double * lsd_scale_region(int * n_out, float * img, int X, int Y, double scale,
  ...
  return LineSegmentDetection(n_out, img, X, Y, scale, sigma_scale, quant,
                              ang_th, log_eps, density_th, n_bins, reg_img,
                              reg_x, reg_y, (Profiler*)0);
}

/*----------------------------------------------------------------------------*/
/** LSD Simple Interface with Scale. */
double * lsd_scale(int * n_out, float * img, int X, int Y, double scale) {
  return lsd_scale_region(n_out, img, X, Y, scale, NULL, NULL, NULL);
}

/*----------------------------------------------------------------------------*/
/** LSD Simple Interface. */
double * lsd(int * n_out, float * img, int X, int Y) {
  // LSD parameters
